...
describe('setupHusky', () => {
    ...
    it('installs and inits husky', async () => {
        ...
        expect(execa).toHaveBeenCalledWith('npm', expect.arrayContaining(['install', 'husky']), expect.anything());
        expect(execa).toHaveBeenCalledWith('npx', expect.arrayContaining(['husky', 'init']), expect.anything());
...
describe('setupGithub', () => {
    ...
    it('creates workflow file', async () => {
        ...
        expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('cocov.yml'), expect.stringContaining('name: Cocov CI'));
    });
});
