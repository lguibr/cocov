...
export async function generateTodo(cwd: string) {
    ...
    let md = '# 📝 Coverage TODO List\n\nGenerated by Cocov. Use this list to reach 100% coverage.\n\n';

    let hasGaps = false;

    for (const [file, cov] of Object.entries(final)) {
        const relPath = path.relative(cwd, file);
        const s = (cov as any).lines || (cov as any).s || {}; // basic check
        // Check for missing lines/branches (simplified)
        // Istanbul format: statementMap, fnMap, branchMap, s, f, b
